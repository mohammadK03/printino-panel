    ...
    <v-container fulid>
      ...
      >
        ...
        <template v-slot:item.postalCost="{ item }">
          <span>{{ toPersianNumber(formatPrice(item.postalCost)) }} ریال</span>
        </template>

        <template v-slot:item.totalDiscount="{ item }">
          <v-icon color="error">trending_down</v-icon>
          <span
            >{{ toPersianNumber(formatPrice(item.totalDiscount)) }} ریال</span
          >
        </template>
...
.orders-order ul li.active a {
  color: #fff;
  background-color: #2b2b2b;
  border: 1px solid #2b2b2b;
}
.orders-order ul li a {
  color: #000;
  display: block;
  font-size: 14px;
  padding: 8px 15px;

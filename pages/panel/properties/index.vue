    ...
    <v-container>
      ...
      >
        ...
        <template v-slot:no-data>
          <span>هنوز ویژگیی اضافه نشده</span>
        </template>
...
export default {
  ...
  data() {
    return {
      ...
      desserts: [],
      valid: true,
      addDialogProperty: false,
      NameRules: [
        (v) => !!v || "نام ویژگی  فیلد اجباری است",
        (v) => (v && v.length <= 20) || "نام ویژگی باید کمتر از 20 نویسه باشد",
      ],
    };
  },
  methods: {
    addDialogparent() {
      (this.propertiesItem = {
        ...
        (this.addDialogProperty = true);
    },
    editProperty(item) {
      // this.getpropertyById(item._id);
      console.log(item);
      this.addDialogProperty = true;
      this.edit = true;
      console.log(item)
      ...
      if (process.browser) {
        ...
        try {
          let resp = await this.$axios.$get(
            url + `api/products/properties/${id}`,
            {
              headers,
            }
          );
    ...
    async properties(lang) {
      ...
      try {
        let data = await this.$axios.$get(
          url + `api/products/properties?lang=${lang}`,
          {
            headers,
          }
        );
